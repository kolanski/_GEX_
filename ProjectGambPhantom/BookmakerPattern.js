...
BookMaker.prototype.OpenPage = function (Url) {
    ...
    myname = this.name;
    this.Phantom.create("--ignore-ssl-errors=yes", "--ssl-protocol=any","--disk-cache=yes","--local-storage-path=c:/bin/storage","--local-storage-quota=100000","--max-disk-cache-size=100000",function (ph) {
      ...
      function (status) {
                ...
                if (status == 'success') {
                    ...
                    page.evaluate(function () { return document.title; }, function (result) { console.log(result); })
                }
            });
...
BookMaker.prototype.OpenPage = function (Url,AfterSuccesUrl) {
    ...
    this.Phantom.create("--ignore-ssl-errors=yes", "--ssl-protocol=any", "--disk-cache=yes", "--local-storage-path=c:/bin/storage", "--local-storage-quota=100000", "--max-disk-cache-size=100000", function (ph) {
      ...
      function (status) {
                ...
                if (status == 'success') {
                    console.log(myname + ' Navigated');
                    page.evaluate(function () { return document.title; }, function (result) { console.log(result); });
                    console.log("navigating to after url");
                    page.open(AfterSuccesUrl);
                }
            });
...
BookMaker.prototype.ParentNavigate = function (Url) {
    ...
    myname1 = this.name;
    this.Phantom.create({ 'parameters': {  'disk-cache': 'yes','cookies-file':'c:/bin/storage/cookies.txt' , 'local-to-remote-url-access': 'yes'}},function (ph) {
        ph.createPage(function (page) {
